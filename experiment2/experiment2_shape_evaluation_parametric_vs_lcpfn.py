...
def evaluate_all_curves_with_metadata_per_scenario(anchor_sizes, lcpfn_model,
                                                          min_points=15, cutoff_percentages=None,
    ...
    scenarios = {
        'Flat': test_flat_curves,
        'MonoConv': test_mono_conv_curves,
        'Peaking': test_peaking_curves,
        'Dipping': test_dipping_curves
    }
    ...
    for scenario_name, curves in scenarios.items():
        ...
        for cutoff_pct in cutoff_percentages:
            if len(curves) > sample_size_per_scenario:
                indices = random.sample(range(len(curves)), sample_size_per_scenario)
            else:
            ...
            args_list = [
                (idx, curves, anchor_sizes, lcpfn_model, min_points, cutoff_pct)
                for idx in indices
            ]

            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                future_to_idx = {
                    executor.submit(evaluate_single_curve_shape, args): args[0]
                ...
                print(
                    f"  Completed {scenario_name} {cutoff_pct * 100:.0f}% - Processed {len(scenario_results)} valid results")

    return pd.DataFrame(all_results)


def evaluate_single_curve_shape(args):
    idx, curves, anchor_sizes, lcpfn_model, min_points, cutoff_pct = args
...
file_path = Path(f'experiment2/learning_curve_extrapolation_results_{sample_size}samples_seed{args.seed}_{model_name_no_ext}.csv')
if file_path.exists():
